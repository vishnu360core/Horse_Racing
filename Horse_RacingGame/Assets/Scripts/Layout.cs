...
public class Layout : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        float xlength = refObject.transform.localScale.x + _gap;
             ...
             case layoutType.horizontal:
                    ...
                    for (int i = 1;i<objects.Count;++i)
                    {
                        if(_direction == layoutDirection.right)
                          objects[i].transform.position = new Vector3(refObject.transform.position.x - i * xlength, refObject.transform.position.y,refObject.transform.position.z);
                        else
                          objects[i].transform.position = new Vector3(refObject.transform.position.x + i * xlength, refObject.transform.position.y, refObject.transform.position.z);
                    }
